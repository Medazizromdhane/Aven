...
import { AlertCircle, ArrowRight, Eye, EyeOff, LockKeyhole, Sparkles } from 'lucide-react';
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[.85fr_1.15fr]">
      <section className="hidden bg-[var(--ink)] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div><p className="mb-5 text-xs font-bold uppercase tracking-[0.18em] text-[#8fd0b2]">{t('welcome')}</p><h2 className="display-type max-w-md text-5xl leading-tight">{t('sideLoginTitle')}</h2><p className="mt-5 max-w-sm leading-7 text-[#afc6bd]">{t('sideLoginCopy')}</p></div>
        <p className="text-sm text-[#78958a]">{t('footerLine')}</p>
      </section>
