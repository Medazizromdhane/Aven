...
import { AlertCircle, ArrowRight, Sparkles, UserRound } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[.85fr_1.15fr]">
      <section className="hidden bg-[var(--ink)] p-12 text-white lg:flex lg:flex-col lg:justify-between"><div className="flex items-center justify-between"><Link href="/" className="brand-lockup text-white"><span className="brand-mark"><Sparkles size={16} /></span>aven</Link><LanguageSwitcher dark /></div><div><p className="mb-5 text-xs font-bold uppercase tracking-[0.18em] text-[#8fd0b2]">{t('startClarity')}</p><h2 className="display-type max-w-md text-5xl leading-tight">{t('nextMove')}</h2><p className="mt-5 max-w-sm leading-7 text-[#afc6bd]">Build a profile that helps the right global opportunities find you.</p></div><p className="text-sm text-[#78958a]">A clearer path to global work.</p></section>
